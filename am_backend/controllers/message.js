...
const getMessagesForReceiver = asyncWrapper(async (req, res) => {
    try {
      ...
      const uniqueSenderIds = await Message.find({ receiver: receiverId }).distinct('sender');

      // Query the Register model to find the firstnames of senders based on their IDs
      const senders = await Register.find({ _id: { $in: uniqueSenderIds } }).select('firstname _id');
  
      // Construct the response data with sender firstnames
      const responseData = senders.map(sender => ({ firstname: sender.firstname,_id : sender._id }));
  ...
  const getMySendedMessages = asyncWrapper(async (req, res) => {
    try {
        ...
        const sentMessages = messages.filter(message => message.sender.toString() === req.body.senderId);
        const receivedMessages = messages.filter(message => message.sender.toString() === req.body.receiverId);
  ...
  const sendCustomerServiceRequest = asyncWrapper(async (req, res) => { 
    ...
    try {
        ...
        const existingMessage = await Chat.findOne({ senderId: req.body.senderId, receiverId: req.body.receiverId });
    
        if (existingMessage) {

          // If the message exists, update the content and timestamp
          if(req.body.receive_msg){
             oldMsg = { receive_msg: req.body.receive_msg, time: req.body.time,date:finalDate};
          }
          if(req.body.sent_msg){
             oldMsg = { sent_msg: req.body.sent_msg, time: req.body.time,date:finalDate};
          }

          data = await Chat.updateOne({ _id:existingMessage._id },{ 
            $push:{ 
              msgs: oldMsg 
          } 
        });
            ...
            if(req.body.receive_msg){
                newMsgList = { receive_msg: req.body.receive_msg, time: req.body.time,date:finalDate};
            }
              if(req.body.sent_msg){
                newMsgList = { sent_msg: req.body.sent_msg, time: req.body.time,date:finalDate};
              }

            data = await Chat.updateOne({ _id:newMsg._id },{ 
              $push:{ 
                msgs: newMsgList 
            } 
          });
...
module.exports = { saveMessage, getMessagesBetweenUsers, getMessagesForReceiver,getSpecificMessages,getMySendedMessages,getSenderMessages,getReceiverMessages,sendCustomerServiceRequest };
